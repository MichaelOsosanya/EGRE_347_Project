...
from level import Sky_Island_Level, Hive_Level
...
class Game_State():
    ...
    def load_level(self):
        if self.level_state == "None":
            ...
            self.current_level = Sky_Island_Level(level_list[0],screen, game_state)
            self.next_level = "Ocean"
        elif self.level_state == "Hive":
            self.level_state = "Hive"
            self.current_level = Hive_Level(level_list[1], screen, game_state)
