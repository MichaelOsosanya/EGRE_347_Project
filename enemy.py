...
class Kraken(Enemy):
    def __init__(self,size,x,y):
        super().__init__(size,x,y, pygame.image.load('../graphics/enemies/ocean_monster/kraken_single.png').convert_alpha())
        self.health = 12
        self.randnum = 1
...
class Hollow(Enemy):
    def __init__(self,size,x,y, level_instance):
        super().__init__(size,x,y, pygame.image.load('../graphics/enemies/final_boss/boss.png').convert_alpha())
        self.health = 14
        self.randnum = 1
        self.level_state = level_instance
    ...
    def update(self, shift):
        ...
        if self.health <= 0:
            self.kill()
            self.level_state.open_next_menu()
